...
//Rare exceptions to indicate a bug in the _alertid code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_alertid = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_alertid]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.alertid.module"
